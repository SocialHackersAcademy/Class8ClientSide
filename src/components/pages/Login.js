...
export class Login extends Component {
  ...
  handleSubmit(event) {
    ...
    if (email != null && password != null) {
      if (
        loginUser('?username=' + email + '&password=' + password, rememberMe)
      ) {
        this.setState({
          message: 'You are logged in',
          messageType: 'alert alert-primary',
        });
      }
    } else {
      this.setState({
        ...
        <div class='row d-flex justify-content-center'>
          <div class='col-8 text-center'>
            <form onSubmit={(event) => this.handleSubmit(event)}>
              ...
              <button
                type='submit'
                className='btn btn-primary btn-block yolonet-submit'
              >
                Submit
              </button>
              <br />
            ...
            <br />
            <br />
            <div class={this.state.messageType} role='alert'>
              {this.state.message}
            </div>
            <br />
            <br />
